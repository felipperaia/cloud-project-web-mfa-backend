...
def cookie_params():
    # Em produção (Render), cookies cross-site exigem SameSite=None; Secure
    # Em dev, usa Lax; Secure=False
    return {
        "httponly": True,
...
async def confirm_email(token: str, response: Response):
    ...
    # Redireciona para o front-end com o token temporário de enrolamento
    temp = create_jwt(
        str(user["_id"]),
        {"type": "mfa_enroll", "username": user["username"]},
        minutes=15,
    )
...
async def mfa_enroll(request: Request):
    ...
    # Gera segredo e URI TOTP compatível com apps como Microsoft Authenticator
    secret = pyotp.random_base32()
    uri = pyotp.totp.TOTP(secret).provisioning_uri(
        name=user["username"], issuer_name="SeuApp"
    )

    # Armazena segredo temporário até a verificação do código TOTP
    await users.update_one({"_id": user["_id"]}, {"$set": {"mfa_secret_temp": secret}})

    # Gera PNG do QR em memória; suporta Pillow e PyPNG
    img = qrcode.make(uri)  # pode retornar Pillow Image ou PyPNGImage
    buf = BytesIO()
    try:
        # Pillow aceita format="PNG"
        img.save(buf, format="PNG")
    except TypeError:
        # PyPNG não aceita 'format', apenas .save(buf)
        img.save(buf)
...
async def get_mfa_enroll_token(current_user=Depends(get_current_user)):
    temp_token = create_jwt(
        ...
        {"type": "mfa_enroll", "username": current_user["username"]},
        minutes=15,
    )
...
async def mfa_enroll_confirm(body: MFAEnrollVerify):
    # Importante: o modelo MFAEnrollVerify deve conter fields `code` e `username`
...
async def login(payload: UserLogin, response: Response, request: Request):
    ...
    if user.get("mfa_enabled"):
        temp = create_jwt(
            str(user["_id"]),
            {"type": "mfa_challenge", "username": user["username"]},
            minutes=settings.MFA_TEMP_EXP_MIN,
        )
        return {"mfa_required": True, "temp_token": temp}

    session = create_jwt(
        str(user["_id"]), {"type": "session"}, minutes=settings.SESSION_EXP_MIN
    )
...
async def mfa_verify(body: MFAVerify, response: Response):
    ...
    is_code = code.isdigit() and mfa_verify_code(
        user["mfa_secret"], code, valid_window=1
    )
    is_backup = False
    if not is_code:
        h = hash_token(code)
        if h in user.get("mfa_backup_codes", []):
            is_backup = True
            await users.update_one(
                {"_id": user["_id"]}, {"$pull": {"mfa_backup_codes": h}}
            )
        ...
        await users.update_one(
            {"_id": user["_id"]},
            {
                "$set": {
                    ...
                    + timedelta(minutes=settings.PASSWORD_RESET_EXP_MIN),
                    "updated_at": now,
                }
            },
        )
